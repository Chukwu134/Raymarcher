...
void main() {
  ...
  worldPos = mVi * pos;
  worldDir = mVi * dir;
  //worldPos = turnMat * worldPos;
  //worldDir = turnMat * worldDir;
  float travelled = 0;
  float steps = 0;
  ...
  //float theta = bias / 10;
  vec3 planeDir = (turnMat * vec4(normalize(vec3(1, .5, .2)), 1)).xyz;
  vec4 planeDirBias = vec4(planeDir, .3);
  while(travelled < 50 && distance > threshold) {
    // eyeSpace = inverse(mVi) * worldPos;
    // if(eyeSpace.z > 5) {
    //   eyeSpace.z = (eyeSpace.z - 5) * -1 + 5;
    // }
    // worldPos = mVi * eyeSpace;

    // if(worldPos.z > 1) {
    //   testingPos.z = (worldPos.z - 1) * -1 + 1;
    // }
    ...
    // testingPos.y = mod(worldPos.y + 2, 5);
    // testingPos.z = mod(worldPos.z + 2, 5);
